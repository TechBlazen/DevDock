...
export class PostgresProvider implements DatabaseProvider {
  ...
  async getApis(): Promise<ApiRow[]> {
    const { rows } = await this.pool.query('SELECT * FROM apis ORDER BY created_at DESC');
    return mapRows<ApiRow>(rows, 'apis');
  }

  async getApiById(id: string): Promise<ApiRow | null> {
    const { rows } = await this.pool.query('SELECT * FROM apis WHERE id = $1', [id]);
    return mapRow<ApiRow>(rows[0], 'apis');
  }

  async getApisByRepo(repoId: string): Promise<ApiRow[]> {
    const { rows } = await this.pool.query(
      'SELECT * FROM apis WHERE source_repo_id = $1 ORDER BY created_at DESC',
      [repoId]
    );
    return mapRows<ApiRow>(rows, 'apis');
  }
  ...
  async getRegistryItems(): Promise<RegistryItemRow[]> {
    const { rows } = await this.pool.query('SELECT * FROM registry_items ORDER BY created_at DESC');
    return mapRows<RegistryItemRow>(rows, 'registry_items');
  }

  async getRegistryItemById(id: string): Promise<RegistryItemRow | null> {
    const { rows } = await this.pool.query('SELECT * FROM registry_items WHERE id = $1', [id]);
    return mapRow<RegistryItemRow>(rows[0], 'registry_items');
  }

  async getRegistryItemBySlug(slug: string): Promise<RegistryItemRow | null> {
    const { rows } = await this.pool.query('SELECT * FROM registry_items WHERE slug = $1', [slug]);
    return mapRow<RegistryItemRow>(rows[0], 'registry_items');
  }
  ...
  async deleteRegistryItem(id: string): Promise<void> {
    await this.pool.query('DELETE FROM registry_items WHERE id = $1', [id]);
  }

  async incrementRegistryInstallCount(id: string, delta: number): Promise<void> {
    await this.pool.query('UPDATE registry_items SET install_count = GREATEST(0, install_count + $1) WHERE id = $2', [delta, id]);
  }

  // ─── Registry versions ────────────────────────────────────────────────────────
  async getRegistryVersions(itemId: string): Promise<RegistryVersionRow[]> {
    const { rows } = await this.pool.query('SELECT * FROM registry_versions WHERE item_id = $1 ORDER BY created_at DESC', [itemId]);
  ...
  async getRegistryInstalls(userId: string): Promise<RegistryInstallRow[]> {
    const { rows } = await this.pool.query('SELECT * FROM registry_installs WHERE user_id = $1 ORDER BY installed_at DESC', [userId]);
  ...
  async getRegistryInstall(itemId: string, userId: string): Promise<RegistryInstallRow | null> {
    const { rows } = await this.pool.query('SELECT * FROM registry_installs WHERE item_id = $1 AND user_id = $2', [itemId, userId]);
  ...
  async updateRegistryInstall(id: string, partial: Partial<RegistryInstallRow>): Promise<RegistryInstallRow | null> {
    const { rows } = await this.pool.query('SELECT * FROM registry_installs WHERE id = $1', [id]);
  ...
  async deleteRegistryInstall(itemId: string, userId: string): Promise<void> {
    await this.pool.query('DELETE FROM registry_installs WHERE item_id = $1 AND user_id = $2', [itemId, userId]);
  }
}
